...
{
  return (unsigned char)stm_load_u8(TXARGS (volatile uint8_t *)addr);
}

short stm_load_short(TXPARAMS volatile short *addr)
{
  convert_16_t val;
  val.u16 = stm_load_u16(TXARGS (volatile uint16_t *)addr);
  return val.s16;
}

unsigned short stm_load_ushort(TXPARAMS volatile unsigned short *addr)
{
  return (unsigned short)stm_load_u16(TXARGS (volatile uint16_t *)addr);
}

int stm_load_int(TXPARAMS volatile int *addr)
{
  convert_32_t val;
  val.u32 = stm_load_u32(TXARGS (volatile uint32_t *)addr);
  return val.s32;
}

unsigned int stm_load_uint(TXPARAMS volatile unsigned int *addr)
{
  return (unsigned int)stm_load_u32(TXARGS (volatile uint32_t *)addr);
}
...
long stm_load_long(TXPARAMS volatile long *addr)
{
  if (sizeof(long) == 4) {
    ...
    convert_64_t val;
    val.u64 = stm_load_u64(TXARGS (volatile uint64_t *)addr);
    return val.s64;
  }
}
...
{
  if (sizeof(long) == 4) {
    return (unsigned long)stm_load_u32(TXARGS (volatile uint32_t *)addr);
  } else {
    return (unsigned long)stm_load_u64(TXARGS (volatile uint64_t *)addr);
  }
}
...
void stm_store_char(TXPARAMS volatile char *addr, char value)
{
  convert_8_t val;
  val.s8 = value;
  stm_store_u8(TXARGS (volatile uint8_t *)addr, val.u8);
}

void stm_store_uchar(TXPARAMS volatile unsigned char *addr, unsigned char value)
{
  stm_store_u8(TXARGS (volatile uint8_t *)addr, (uint8_t)value);
}
...
void stm_store_short(TXPARAMS volatile short *addr, short value)
{
  convert_16_t val;
  val.s16 = value;
  stm_store_u16(TXARGS (volatile uint16_t *)addr, val.u16);
}

void stm_store_ushort(TXPARAMS volatile unsigned short *addr, unsigned short value)
{
  stm_store_u16(TXARGS (volatile uint16_t *)addr, (uint16_t)value);
}
...
void stm_store_int(TXPARAMS volatile int *addr, int value)
{
  convert_32_t val;
  val.s32 = value;
  stm_store_u32(TXARGS (volatile uint32_t *)addr, val.u32);
}
...
void stm_store_long(TXPARAMS volatile long *addr, long value)
{
  if (sizeof(long) == 4) {
    ...
    convert_64_t val;
    val.s64 = value;
    stm_store_u64(TXARGS (volatile uint64_t *)addr, val.u64);
  }
}
...
void stm_store_ulong(TXPARAMS volatile unsigned long *addr, unsigned long value)
{
  if (sizeof(long) == 4) {
    ...
    stm_store_u64(TXARGS (volatile uint64_t *)addr, (uint64_t)value);
  }
}
